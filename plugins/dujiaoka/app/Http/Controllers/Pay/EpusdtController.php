...
class EpusdtController extends PayController
{
    public function gateway(string $payway, string $orderSN)
    {
        try {
            ...
            //构造要请求的参数数组，无需改动
            $parameter = [
                'pid' => (string)$this->payGateway->merchant_id,
            ...
            $parameter['signature'] = $this->epusdtSign($parameter, $this->payGateway->merchant_key);
            $client = new Client([
    ...
    public function notifyUrl(Request $request)
    {
        ...
        $signature = $this->epusdtSign($data, $payGateway->merchant_key);
